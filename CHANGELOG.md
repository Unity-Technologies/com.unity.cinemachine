...
- Information sign and button added to virtual cameras that have components or extensions that require input (e.g. freelook, orbital transposer, POV), when the InputSystem package is installed and the vcam does not have already have CinemachineInputProvider component. When clicked, the button adds CinemachineInputProvider with a default generic XY axis control to the vcam.
