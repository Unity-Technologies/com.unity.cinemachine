...
- Added simplified modes for impulse generation and added secondary reaction settings to Impulse Listener.
...
- Added Draggable Game Window Guides toggle in Cinemachine preferences. When disabled, the Game Window guides only provide visualization.
- Added button to virtual camera inspectors to auto-generate CinemachineInputProvider component when the component is missing.
...
- Bugfix: 3rdPersonFollow damping operated in world space instead of camera space.
- Bugfix: 3rdPersonFollow stuttered when Z damping was set high.
