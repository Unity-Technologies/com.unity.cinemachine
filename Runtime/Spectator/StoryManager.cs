...
The Story Thread, which in essence follows a single character (or group of characters).  The Story thread has the following attributes: 
...
Urgency - decays while on-screen, increases otherwise.  Events and action states influence this heavily.  
The Urgency evolution function is non-trivial and is configurable

Story Threads will be kept sorted by urgency.  A high-quality shot covering the most urgent Story Thread will be selected.

A story thread follows an interesting subject (or group of subjects).  "Interesting" is defined by the dev.
Story threads are created as interesting subjects are created.   This should be a call to the Story Manager
...
Once we have a collection of shots that cover the desired Story Thread (rule 1: What we want to see), 
the Director must then choose from among the available shots covering the Story Thread (rules 2 and 3: How we want to see it).  
The shots themselves will be rated for quality, as before, but a new element will be added: the quality of the transition. 
In other words: is this a good cut?
*/
...
public class StoryManager 
{
    ...
    public class StoryThread
    {
        ...
        // Last on-screen duration
        // Interest level - Controlled by the dev, to focus on a specific thread he judges relevant 
        // Urgency - decays while on-screen, increases otherwise.  Events and action states influence this heavily.  The Urgency evolution function is non-trivial and is configurable
    }
