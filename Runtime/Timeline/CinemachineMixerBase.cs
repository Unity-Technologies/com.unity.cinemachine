...
internal abstract class CinemachineMixerBase<T> : PlayableBehaviour
{
    ...
    public override void ProcessFrame(Playable playable, FrameData info, object playerData)
    {
        ...
        if (playerData == null || playerData is T == false)
            return;
        ...
        ProcessBlendFrame(info, target, camA, camB, weightB);
        ...
        if (m_ScrubbingCacheHelper != null && TargetPositionCache.CacheMode != TargetPositionCache.Mode.Disabled)
        {
            ...
            if (clipIndexA >= 0)
                m_ScrubbingCacheHelper.ScrubToHere(
                    ...
                    (float)playable.GetInput(clipIndexA).GetTime(), Vector3.up);
            if (clipIndexB >= 0)
                m_ScrubbingCacheHelper.ScrubToHere(
                    (float)GetMasterPlayableDirector().time, clipIndexB, isNewB && weightB > 0.99f,
                    (float)playable.GetInput(clipIndexB).GetTime(), Vector3.up);
        }
#endif
    }

    protected float GetDeltaTime(float deltaTime)
    {
    ...
    protected abstract void ProcessBlendFrame(FrameData info, T target, ICinemachineCamera camA, ICinemachineCamera camB, float blendWeight);
}
#endif
