...
namespace Cinemachine
{
    ...
    public class CinemachineVirtualCamera : CinemachineVirtualCameraBase
    {
        ...
        public override bool DrawSceneTools(Color activeColor, Color defaultColor)
        {
            ...
            if (Utility.CinemachineSceneToolUtility.FoVToolIsOn)
            {
                var cameraPosition = State.FinalPosition;
                var cameraRotation = State.FinalOrientation;
                var cameraForward = cameraRotation * Vector3.forward;
                
                UnityEditor.EditorGUI.BeginChangeCheck();
                var fieldOfView = UnityEditor.Handles.ScaleSlider(m_Lens.FieldOfView, cameraPosition, cameraForward, 
                    cameraRotation, UnityEditor.HandleUtility.GetHandleSize(cameraPosition), 0.1f);
                if (UnityEditor.EditorGUI.EndChangeCheck())
                {
                    ...
                    doRepaint = true;
                    UnityEditor.Undo.RecordObject(this, "Changed FOV using handle in scene view.");
                }
                else
                {
                ...
                UnityEditor.Handles.Label(cameraPosition + 
                    cameraForward * UnityEditor.HandleUtility.GetHandleSize(cameraPosition), 
                    "FOV (" + m_Lens.FieldOfView.ToString("F1") + ")", labelStyle);
            }

            if (Utility.CinemachineSceneToolUtility.FarNearClipToolIsOn)
            {
                var cameraPosition = State.FinalPosition;
                var cameraRotation = State.FinalOrientation;
                var cameraForward = cameraRotation * Vector3.forward;
                var nearClipPos = cameraPosition;
                var nearClipSize = UnityEditor.HandleUtility.GetHandleSize(nearClipPos);
                var farClipPos = cameraPosition + cameraForward * nearClipSize * 2f;
                var farClipSize = UnityEditor.HandleUtility.GetHandleSize(farClipPos);
                
                UnityEditor.EditorGUI.BeginChangeCheck();
                var nearClipPlane = UnityEditor.Handles.ScaleSlider(m_Lens.NearClipPlane, 
                    nearClipPos, cameraForward, 
                    cameraRotation, nearClipSize, 0.1f);
                var farClipPlane = UnityEditor.Handles.ScaleSlider(m_Lens.FarClipPlane, 
                    farClipPos, cameraForward, 
                    cameraRotation, farClipSize, 0.1f);
                if (UnityEditor.EditorGUI.EndChangeCheck())
                {
                    m_HandleIsBeingDragged = true;
                    m_Lens.NearClipPlane = nearClipPlane;
                    m_Lens.FarClipPlane = farClipPlane;
                    
                    doRepaint = true;
                    UnityEditor.Undo.RecordObject(this, "Changed clip plane using handle in scene view.");
                }
                else
                {
                    m_HandleIsBeingDragged = false;
                ...
                UnityEditor.Handles.Label(nearClipPos, 
                    "Near Clip Plane (" + m_Lens.NearClipPlane.ToString("F1") + ")", labelStyle);
                UnityEditor.Handles.Label(farClipPos, 
                    "Far Clip Plane (" + m_Lens.FarClipPlane.ToString("F1") + ")", labelStyle);
            }
