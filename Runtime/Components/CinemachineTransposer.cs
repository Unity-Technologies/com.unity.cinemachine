    ...
    public class CinemachineTransposer : CinemachineComponentBase
    {
            ...
            out Vector3 outTargetPosition, out Quaternion outTargetOrient)
        {
            ...
            if (prevStateValid)
            {
                ...
                {
                    float t = VirtualCamera.DetachedTargetDamp(1, m_AngularDamping, deltaTime);
                ...
                else
                {
                    ...
                    for (int i = 0; i < 3; ++i)
                        if (relative[i] > 180)
                            relative[i] -= 360;
                    relative = VirtualCamera.DetachedTargetDamp(relative, AngularDamping, deltaTime);
            ...
            if (prevStateValid)
            {
                ...
                localDelta = VirtualCamera.DetachedTargetDamp(localDelta, Damping, deltaTime);
            ...
            Vector3 cameraFwd, Vector3 up, Vector3 actualTargetPos)
        {
            ...
            if (VirtualCamera.TargetAttachment > 1 - Epsilon)
            {
