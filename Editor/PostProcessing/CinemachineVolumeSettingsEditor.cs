        ...
        if (m_FocusTracksTarget.boolValue)
        {
            ...
            if (!valid)
                EditorGUILayout.HelpBox(
                    "Focus Tracking requires an active DepthOfField/FocusDistance "
                        + "effect and FocusMode set to Physical Camera in the profile",
                    MessageType.Warning);
            else
            {
        ...
        if (m_Profile.objectReferenceValue == null)
        {
            ...
            EditorGUILayout.HelpBox(
                "Assign an existing Volume Profile by choosing an asset, or create a new one by "
                    + "clicking the \"New\" button.\nNew assets are automatically put in a folder "
                    + "next to your scene file. If your scene hasn't been saved yet they will be "
                    + "created at the root of the Assets folder.",
                MessageType.Info);
        }
        else
        {
